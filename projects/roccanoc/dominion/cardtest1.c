...
int main(int argc, char** argv)
{
	...
	expected = control.handCount[playerA] + 1; //2 cards gained, 1 card played
	...
	expected = control.deckCount[playerA] - 2;
	...
	for (int i = 0; i < control.handCount[playerA]; i++)
	{
		tcard = control.hand[playerA][i];
	...
	for (int i = 0; i < test.handCount[playerA]; i++)
	{
		tcard = test.hand[playerA][i];
		if (tcard == copper || tcard == silver || tcard == gold) 
		{
			testTreasure++;
		}

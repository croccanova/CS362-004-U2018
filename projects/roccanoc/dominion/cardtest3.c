...
int main(int argc, char** argv)
{
	...
	printf("Running tests for outpost card\n");

	cardEffect(outpost, choice1, choice2, choice3, &test, handPos, &bonus);

	//test hand count
	printf("\nTest hand count for player A\n");
	expected = control.handCount[playerA] - 1; //0 cards gained, 1 card played
	...
	expected = control.deckCount[playerA];
	actual = test.deckCount[playerA];
	printf("Expected: %d	Actual: %d\n", expected, actual);
	isTrue(expected, actual);

	//test outpost flag
	//expected to fail due to introduced bug
	printf("\nTesting that outpost flag is set\n");
	expected = control.outpostPlayed + 1;
	actual = test.outpostPlayed;
	printf("Expected: %d	Actual: %d\n", expected, actual);
	isTrue(expected, actual);

/*
	...
	CS362-400
	Assignment 4
	Random tester for the Great Hall card
*/
...
void greatHallTest(struct gameState control, int failCount)
{
	...
	cardEffect(great_hall, c1, c2, c3, &test, handPos, &bonus);

	//test hand count
	printf("\nTesting hand count\n");
	expected = control.handCount[playerNum]; //1 cards gained, 1 card played
	...
	//test deck count
	printf("\nTesting deck count\n"); //deck decreases by 1
	expected = control.deckCount[playerNum] - 1;
